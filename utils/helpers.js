...
export function buildRoutePolyline(
  ...
  if (historyPoints.length > 1) {
    const withCurrent = currentLocation
      ? [...historyPoints, sanitizeCoordinate(currentLocation)].filter(Boolean)
      : historyPoints;
...
export function buildSummary(trip = {}) {
  ...
  const traveledDistanceKm = calculatePathDistanceKm(
    historyPoints.length > 1 ? historyPoints : routePoints,
  );
  ...
  const activeDurationMs = trip.startedAt
    ? Math.max(
        ...
        (trip.endedAt || Date.now()) -
          trip.startedAt -
          Number(trip.pausedDurationMs || 0),
      )
    : 0;
  const averageSpeed = activeDurationMs
    ? traveledDistanceKm / (activeDurationMs / 1000 / 3600)
    : 0;
  const completedStops = (trip.stops || []).filter((stop) =>
    Boolean(stop.completed),
  ).length;
  const totalStops = (trip.stops || []).length;
  const routeEfficiency = plannedRouteDistanceKm
    ? Math.min(
        100,
        Math.round((traveledDistanceKm / plannedRouteDistanceKm) * 100),
      )
    : totalStops
...
export function findCurrentAndNextStops(stops = [], currentLocation = null) {
  ...
  const completedSet = new Set(
    orderedStops.filter((stop) => stop.completed).map((stop) => stop.id),
  );
  const currentIndex = orderedStops.findIndex(
    (stop) => !completedSet.has(stop.id),
  );
  const currentStop = currentIndex >= 0 ? orderedStops[currentIndex] : null;
  const nextStop =
    currentIndex >= 0 ? orderedStops[currentIndex + 1] || null : null;
