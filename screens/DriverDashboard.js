...
export default function DriverDashboard() {
  ...
  const liveDurationMs = useMemo(() => {
    ...
    const pausedNow = trip.pausedAt
      ? Math.max(0, Date.now() - trip.pausedAt)
      : 0;
  ...
  const metrics = useMemo(() => {
    ...
    return (
      trip.summary || {
        ...
        averageSpeed: 0,
        completedStops:
          trip.stops?.filter((stop) => stop.completed).length || 0,
    ...
    try {
      ...
      if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
        if (trip?.currentLocation) {
          ...
          throw new Error(
            "Provide coordinates or start the trip to use live GPS.",
          );
        }
      }
  ...
  const canStart = Boolean(
    trip && (trip.status === "Idle" || trip.status === "Paused"),
  );
      ...
      >
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.title}>
              Manage the trip from one clean dashboard.
            </Text>
          </View>
        ...
        {!trip || trip.status === "Completed" ? (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>
              {trip?.status === "Completed" ? "Create New Trip" : "Create Trip"}
            </Text>
            <Text style={styles.muted}>
              Enter the trip details before starting live GPS tracking.
            </Text>
            <TextInput
              value={form.driverName}
              onChangeText={(value) =>
                setForm((prev) => ({ ...prev, driverName: value }))
              }
            ...
            <TextInput
              ...
              onChangeText={(value) =>
                setForm((prev) => ({ ...prev, busNumber: value.toUpperCase() }))
              }
            ...
            <TextInput
              ...
              onChangeText={(value) =>
                setForm((prev) => ({ ...prev, routeNumber: value }))
              }
            ...
            <TextInput
              ...
              onChangeText={(value) =>
                setForm((prev) => ({ ...prev, shiftTiming: value }))
              }
        ...
        {trip && trip.status !== "Completed" ? (
          <>
            <View style={styles.controlsCard}>
              ...
              <View style={styles.metricsGrid}>
                <StatCard
                  label="Driver Code"
                  value={trip.joinCode}
                  hint="Share with students"
                  accent="#111827"
                />
                <StatCard
                  label="Status Timer"
                  value={formatDuration(metrics?.durationMs || liveDurationMs)}
                  hint="Elapsed trip time"
                  accent="#111827"
                />
                <StatCard
                  label="Distance"
                  value={formatKm(metrics?.totalDistanceKm || 0)}
                  hint="GPS path length"
                  accent="#111827"
                />
                <StatCard
                  label="Stops"
                  value={`${metrics?.completedStops || 0}/${metrics?.totalStops || 0}`}
              ...
              <View style={styles.infoRow}>
                <View style={styles.infoPill}>
                  ...
                  <Text style={styles.infoValue}>
                    {currentStop?.name || "Waiting for movement"}
                  </Text>
                </View>
                <View style={styles.infoPill}>
                  <Text style={styles.infoLabel}>Next stop</Text>
                  <Text style={styles.infoValue}>
                    {nextStop?.name || "Waiting for movement"}
                  </Text>
                </View>
              </View>
            </View>
            ...
            <DelayToggle
              ...
              onChange={(value) =>
                actions.setDelay({ delayed: value, reason: trip.delayReason })
              }
              onChangeReason={(text) =>
                actions.setDelay({ delayed: true, reason: text })
              }
            />
            ...
            <View style={styles.card}>
              ...
              <Text style={styles.muted}>
                Use live GPS if fields are empty, or manually enter exact
                coordinates.
              </Text>
              <TextInput
                value={stopDraft.name}
                onChangeText={(value) =>
                  setStopDraft((prev) => ({ ...prev, name: value }))
                }
              ...
              <View style={styles.twoCol}>
                <TextInput
                  ...
                  onChangeText={(value) =>
                    setStopDraft((prev) => ({ ...prev, latitude: value }))
                  }
                ...
                <TextInput
                  ...
                  onChangeText={(value) =>
                    setStopDraft((prev) => ({ ...prev, longitude: value }))
                  }
              ...
              <View style={styles.twoCol}>
                <TextInput
                  ...
                  onChangeText={(value) =>
                    setStopDraft((prev) => ({ ...prev, scheduledTime: value }))
                  }
                ...
                <TextInput
                  ...
                  onChangeText={(value) =>
                    setStopDraft((prev) => ({ ...prev, order: value }))
                  }
                  placeholder="Order"
                  placeholderTextColor="#94A3B8"
            ...
            <View style={styles.mapCard}>
              <View style={styles.rowBetween}>
                <View style={styles.flexOne}>
                  ...
                  <Text style={styles.muted}>
                    {trip.driverName} • Bus {trip.busNumber} • Route{" "}
                    {trip.routeNumber}
                  </Text>
                </View>
                <View style={styles.statusChip}>
        ...
        {trip && trip.status === "Completed" ? (
          <View style={styles.card}>
            ...
            <Text style={styles.muted}>
              Trip completed successfully. Start a new trip from the form above.
            </Text>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Total Distance</Text>
              <Text style={styles.summaryValue}>
                {formatKm(trip.summary?.totalDistanceKm || 0)}
              </Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Duration</Text>
              <Text style={styles.summaryValue}>
                {formatDuration(trip.summary?.durationMs || 0)}
              </Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Stops</Text>
              <Text style={styles.summaryValue}>
                {trip.summary?.completedStops || 0} /{" "}
                {trip.summary?.totalStops || 0}
              </Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Average Speed</Text>
              <Text style={styles.summaryValue}>
                {Number(trip.summary?.averageSpeed || 0).toFixed(2)} km/h
              </Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Route Efficiency</Text>
              <Text style={styles.summaryValue}>
                {trip.summary?.routeEfficiency || 0}%
              </Text>
            </View>
          </View>
        ) : null}
