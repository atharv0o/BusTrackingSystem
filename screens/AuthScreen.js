...
export default function AuthScreen({ navigation, route }) {
  ...
  const roleLabel = useMemo(() => (role === 'driver' ? 'Driver' : 'Student'), [role]);
    ...
    <GradientShell>
      <View style={styles.container}>
        ...
        <Text style={styles.subtitle}>
          {mode === 'login'
            ? 'Log in locally to access your dashboard.'
            : 'Create a local profile for this device. No Firebase required.'}
        </Text>
        ...
        <TouchableOpacity disabled={loading} onPress={submit} style={[styles.primaryBtn, loading && styles.disabled]}>
          <Text style={styles.primaryBtnText}>{loading ? 'Please wait...' : mode === 'login' ? 'Login' : 'Create Account'}</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => navigation.navigate('Launch')} style={styles.secondaryBtn}>
